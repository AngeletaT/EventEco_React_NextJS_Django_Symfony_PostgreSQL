...
class IsOrganizer implements EventSubscriberInterface
{
    ...
    public function onKernelRequest(RequestEvent $event): void
    {
        ...
        // Solo se protege si la ruta comienza con "/e_symfony/api/organizer"
        if (strpos($path, '/e_symfony/api/organizer') !== 0) {
            return;
        }

        // Definir las rutas públicas que NO requieren token (con el prefijo completo)
        $publicRoutes = [
            '/e_symfony/api/organizer/login',
            '/e_symfony/api/organizer/register',
        ];
