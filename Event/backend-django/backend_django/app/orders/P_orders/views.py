...
from .serializers import P_OrderDetailSerializer, P_TicketUnitSerializer, P_ClientDashboardSerializer
...
class P_OrderDetailView(APIView):
     """
     ...
     @swagger_auto_schema(
          ...
          responses={200: P_OrderDetailSerializer()}
     )
     def get(self, request, idorder):
          order = get_object_or_404(P_Order, idorder=idorder)
          serializer = P_OrderDetailSerializer(order)
          return Response(serializer.data, status=status.HTTP_200_OK)

class P_OrderCreateView(APIView):
     """
     ...
     @swagger_auto_schema(
          ...
          responses={201: P_OrderDetailSerializer(), 400: "Datos inválidos", 401: "No autorizado"}
     )
     @transaction.atomic
     def post(self, request):
          ...
          for ticket in tickets:
               ...
               ticket_info = P_TicketInfo.objects.filter(idticketinfo=idticketinfo).first()
               if not ticket_info:
               ...
               for entry in entries:
                    ...
                    for complement_id in complements:
                         complement = P_Complement.objects.filter(idcomplement=complement_id).first()
                         if complement:
...
class P_TicketUnitUpdateView(APIView):
     """
     ...
     @swagger_auto_schema(
          ...
          responses={200: P_TicketUnitSerializer(), 400: "Datos inválidos", 404: "TicketUnit no encontrado"}
     )
     def put(self, request, ticketunitid):
          """
          PUT: http://localhost:8099/p_django/api/ticketunit/{ticketunitid}
          """
          # 🔍 Buscar el TicketUnit
          ticketunit = get_object_or_404(P_TicketUnit, idticketunit=ticketunitid)

          # 📝 Validar los datos de entrada con el serializer
          serializer = P_TicketUnitSerializer(ticketunit, data=request.data, partial=True)
...
class P_ClientDashboardView(APIView):
     """
     ...
     def get(self, request):
          ...
          orders = P_Order.objects.filter(idclient=idclient)
          ...
          serializer = P_ClientDashboardSerializer({"orders": orders}, context={"idclient": idclient})
