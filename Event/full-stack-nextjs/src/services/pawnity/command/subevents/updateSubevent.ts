...
export const updateSubevent = async ({ idsubevents, subeventData }: { idsubevents: number; subeventData: Object }) => {
    try {
        ...
        const response = await symfonyAPI_P.put(`organizer/subevent/${idsubevents}`, subeventData, { headers });
...
export const toggleSubevent = async ({ idsubevents }: { idsubevents: number }) => {
    try {
        ...
        const response = await symfonyAPI_P.post(`organizer/subevent/${idsubevents}`, {}, { headers });
        return response.data;
    } catch (error) {
